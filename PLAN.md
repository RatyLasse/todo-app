...
Priority estimates the consequence of delay from the title: `high` for safety,
health, security, access, financial, major practical, or firm-deadline risks;
`low` for clearly optional, recreational, relaxing, or no-rush work with no
meaningful consequence if delayed; otherwise `medium` for ordinary obligations,
routine work, or genuinely ambiguous titles. The prompt includes contrasting
examples so semantic cases such as a broken car tire versus relaxing in a hammock
do not collapse to `medium` merely because they lack explicit urgency words.
...
- defines each allowed priority and label, with consequence-based priority guidance and contrasting examples;
