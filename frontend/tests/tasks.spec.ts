...
test("create, edit, cancel, complete, reopen, and delete a task", async ({ page }) => {
  ...
  await expect(page.getByRole("article", { name: "Book dentist appointment", exact: true })).toBeVisible();
  await expect(page.getByRole("article", { name: "Book dentist appointment", exact: true }).getByText("high priority", { exact: true })).toBeVisible();
  await expect(page.getByRole("button", { name: "Undo", exact: true })).toHaveCount(0);
  const reverted = page.getByRole("article", { name: "Book dentist appointment", exact: true });
  await reverted.getByRole("button", { name: "Book dentist appointment", exact: true }).click();
  await page.getByRole("textbox", { name: "Task title" }).fill("Book annual checkup");
  await page.getByRole("combobox", { name: "Priority" }).selectOption("low");
  await page.getByRole("combobox", { name: "Label", exact: true }).selectOption("personal");
  await page.getByRole("button", { name: "Save changes" }).click();
  const updatedAgain = page.getByRole("article", { name: "Book annual checkup", exact: true });
  await expect(updatedAgain).toBeVisible();
  await updatedAgain.getByRole("checkbox").click();
  await expect(updatedAgain.getByRole("checkbox")).toBeChecked();
  await expect(updatedAgain.getByRole("heading", { level: 3 })).toHaveCSS("text-decoration-line", "line-through");
  await expect(updatedAgain.getByText("Done", { exact: true })).toHaveCount(0);
  await page.reload();
  await expect(updatedAgain.getByRole("checkbox")).toBeChecked();
  await updatedAgain.getByRole("checkbox").click();
  await expect(updatedAgain.getByRole("checkbox")).not.toBeChecked();
  await expect(updatedAgain.getByText("Done", { exact: true })).toHaveCount(0);

  page.once("dialog", (dialog) => void dialog.dismiss());
  await updatedAgain.getByRole("button", { name: "Delete Book annual checkup", exact: true }).click();
  await expect(updatedAgain).toBeVisible();
  page.once("dialog", (dialog) => void dialog.accept());
  await updatedAgain.getByRole("button", { name: "Delete Book annual checkup", exact: true }).click();
  await expect(page.getByRole("button", { name: "Undo", exact: true })).toBeVisible();
  await page.getByRole("button", { name: "Undo", exact: true }).click();
  await expect(page.getByRole("article", { name: "Book annual checkup", exact: true })).toBeVisible();
  await expect(page.getByRole("button", { name: "Undo", exact: true })).toHaveCount(0);
  page.once("dialog", (dialog) => void dialog.accept());
  await page.getByRole("button", { name: "Delete Book annual checkup", exact: true }).click();
  await expect(page.getByRole("heading", { name: "A fresh start" })).toBeVisible();
  await page.reload();
  await expect(page.getByRole("heading", { name: "A fresh start" })).toBeVisible();
